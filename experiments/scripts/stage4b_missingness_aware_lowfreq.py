...
STAGE4A = host_path(r"E:\codex_file\新方向阶段4")
OUT = host_path(r"E:\codex_file\新方向阶段4B_低频定向Reward")
...
from process_kirc_3omics import FUNCTIONAL_EFFECTS, clean_gene, sample_type  # noqa: E402
...
MUTATION_ABSENCE_AUDIT = OUT / "01_mutation_audit" / "mutation_absence_semantics_audit.md"
RAW_MUTATION = PROJECT / "data" / "raw" / "KIRC_mc3.txt"
LOW_FREQUENCY_TARGET_TAU = 10.0
...
def mkdirs():
    dirs = [
        "02_reward_design",
        "03_smoke_test/Recovery",
        "03_smoke_test/Discovery",
        "03_smoke_test/Robustness",
        "04_formal_runs/Recovery",
        "04_formal_runs/Discovery",
        "04_formal_runs/Robustness",
        "05_rankings",
        "06_metrics",
        "07_comparison",
        "08_reports/figures",
...
def low_frequency_target_score(count, tau=LOW_FREQUENCY_TARGET_TAU):
    if count is None:
        return 0.0
    try:
        n = float(count)
    ...
    if 2 <= n <= 18:
        return 1.0
    return float(np.clip(math.exp(-((n - 18) / tau)), 0.0, 1.0))


def load_raw_mutation_gene_sets():
    log_access(RAW_MUTATION, "read raw KIRC MC3 mutation calls for MutationEvidenceStatus", "read", "project_raw_mutation", True)
    df = pd.read_csv(RAW_MUTATION, sep="\t", usecols=["sample", "gene", "effect"])
    df = df[df["sample"].map(sample_type) == "01"].copy()
    df["Gene"] = clean_gene(df["gene"])
    df = df.dropna(subset=["Gene"])
    df["effect_key"] = df["effect"].astype(str).str.strip().str.lower()
    raw_any = set(df["Gene"].astype(str).str.upper())
    functional = set(df[df["effect_key"].isin(FUNCTIONAL_EFFECTS)]["Gene"].astype(str).str.upper())
    return raw_any, functional, {
        "raw_rows": int(len(df)),
        "raw_tumor_genes": int(len(raw_any)),
        "raw_functional_genes": int(len(functional)),
        "raw_tumor_samples": int(df["sample"].nunique()),
    }


def mutation_evidence_status(gene, count, raw_any_tumor_genes):
    if count is None:
        return "unknown"
    try:
        n = int(count)
        ...
        return "unknown"
    if n >= 1:
        return "observed_functional_mutation"
    if n == 0:
        key = str(gene).upper()
        if key in raw_any_tumor_genes:
            return "no_functional_but_raw_record"
        return "no_raw_mutation_record"
    return "unknown"
...
class Stage4RewardModel:
    def __init__(self, gene_names, node_features, feature_columns, train_driver_genes, ppi_degree, grn_degree, raw_any_tumor_genes):
        ...
        self.mutation_evidence_status = np.asarray(
            [
                mutation_evidence_status(gene, int(count), raw_any_tumor_genes)
                for gene, count in zip(self.gene_names, self.mutation_patient_count)
            ],
            dtype=object,
        )
        self.low_frequency_target_score = np.asarray(
            [low_frequency_target_score(int(count)) for count in self.mutation_patient_count],
            dtype=np.float64,
        )
    ...
    def _static_components(self):
        ...
        weak_novelty = 0.03 * np.asarray([0.0 if g.upper() in self.train_driver_set else 1.0 for g in self.gene_names])
        mutation_target_reward = self.low_frequency_target_score * self.nonmutation_support
        discovery = mutation_target_reward + weak_novelty
        ...
        return pd.DataFrame(
            {
                ...
                "MutationEvidenceStatus": self.mutation_evidence_status,
                "LowFrequencyTargetScore": self.low_frequency_target_score,
                "MutationTargetReward": mutation_target_reward,
                ...
                "supported_low_frequency": self.low_frequency_candidate & (self.nonmutation_support >= self.support_threshold),
    ...
    def component_for_action(self, action, selected_before):
        ...
        return {
            ...
            "MutationEvidenceStatus": str(row["MutationEvidenceStatus"]),
            "LowFrequencyTargetScore": float(row["LowFrequencyTargetScore"]),
            "MutationTargetReward": float(row["MutationTargetReward"]),
...
class Stage4DeepQNetwork(DeepQNetwork):
    ...
    def step(self, network, action, gene_num, gene_name, weights):
        ...
        self.last_reward_components = {
            ...
            "MutationEvidenceStatus": comp["MutationEvidenceStatus"],
            "LowFrequencyTargetScore": comp["LowFrequencyTargetScore"],
            "MutationTargetReward": comp["MutationTargetReward"],
...
TRACE_FIELDS = [
    ...
    "MutationFrequencyPct", "MutationGroup", "MutationEvidenceStatus", "LowFrequencyTargetScore",
    "MutationTargetReward", "nonmutation_support", "very_low_supported",
...
def build_run(policy, seed, run_dir, message_edge_path, reward_model, smoke=False):
    ...
    if run_dir.exists():
        raise RuntimeError(f"Refusing to overwrite incomplete Stage4-B run directory: {run_dir}")
    ...
    config = {
        "stage": "Stage4-B Missingness-Aware Low-Frequency-Targeted MORL",
        "policy": policy,
        ...
        "reward_vector": ["R_relevance", "R_discovery_B", "R_robustness"],
        "historical_test_read": False,
        "external_validation_read": False,
        "validation_labels_used_for_reward": False,
        "morl_started": False,
        "primary_endpoint": "Validation NDCG@150",
        "stage4A_baseline_dir": display(STAGE4A),
        "N0_mutation_target_reward": 0.0,
        "low_frequency_target_tau": LOW_FREQUENCY_TARGET_TAU,
        "device": str(device),
...
def load_stage4_reward_model():
    run_dir = OUT / "02_reward_design" / "_environment_probe"
    run_dir.mkdir(parents=True, exist_ok=True)
    ...
    raw_any_tumor_genes, _, raw_stats = load_raw_mutation_gene_sets()
    write_json(OUT / "02_reward_design" / "raw_mutation_gene_set_stats.json", raw_stats)
...
def run_reward_unit_tests(model, score_rows):
    checks = []

    def add(name, actual, expected=None, tol=1e-9):
        if expected is None:
            passed = bool(actual)
        else:
            passed = abs(float(actual) - float(expected)) <= tol
        checks.append({"check": name, "actual": actual, "expected": expected, "status": "PASS" if passed else "FAIL"})
    ...
    for n, expected in expected_scores.items():
        add(f"score_N_{n}", low_frequency_target_score(n), expected, tol=1e-6)
    add("score_nan_zero", low_frequency_target_score(float("nan")), 0.0)
    add("score_inf_zero", low_frequency_target_score(float("inf")), 0.0)
    add("all_scores_between_0_and_1", all(0.0 <= row["LowFrequencyTargetScore"] <= 1.0 for row in score_rows))
    add("group_N_0", classify_mutation_frequency(0) == "very_low")
    add("group_N_1", classify_mutation_frequency(1) == "very_low")
    add("group_N_2", classify_mutation_frequency(2) == "low_frequency")
    add("group_N_18", classify_mutation_frequency(18) == "low_frequency")
    add("group_N_19", classify_mutation_frequency(19) == "high_frequency")
    st = model.static
    add(
        "count_ge_1_observed_functional",
        bool((st.loc[st["MutationPatientCount"] >= 1, "MutationEvidenceStatus"] == "observed_functional_mutation").all()),
    )
    add(
        "count_zero_not_confirmed_true_zero",
        "confirmed_true_zero" not in set(st.loc[st["MutationPatientCount"] == 0, "MutationEvidenceStatus"]),
    )
    add(
        "N0_mutation_target_reward_zero",
        bool((st.loc[st["MutationPatientCount"] == 0, "MutationTargetReward"] == 0.0).all()),
    )
    add("reward_finite", bool(np.isfinite(st[["R_relevance_static", "R_discovery_static", "R_robustness_static"]].to_numpy()).all()))
    add("discovery_within_bounds", bool(((st["R_discovery_static"] >= 0.0) & (st["R_discovery_static"] <= 1.0)).all()))
    passed = all(row["status"] == "PASS" for row in checks)
    return {"status": "PASS" if passed else "FAIL", "checks": checks}


def write_reward_dry_run(model):
    rows = []
    st = model.static.copy()
    examples = [
        ("N=0 no raw record", st[(st["MutationPatientCount"] == 0) & (st["MutationEvidenceStatus"] == "no_raw_mutation_record")]),
        ("N=0 raw nonfunctional record", st[(st["MutationPatientCount"] == 0) & (st["MutationEvidenceStatus"] == "no_functional_but_raw_record")]),
        ("N=1 observed functional", st[st["MutationPatientCount"] == 1]),
        ("N=2-18 low frequency", st[(st["MutationPatientCount"] >= 2) & (st["MutationPatientCount"] <= 18)]),
        ("N>=19 high frequency", st[st["MutationPatientCount"] >= 19]),
    ]
    for label, sub in examples:
        if len(sub) == 0:
            rows.append({"ExampleClass": label, "Gene": "", "notes": "no gene found"})
            continue
        row = sub.sort_values(["nonmutation_support", "R_discovery_static"], ascending=False).iloc[0]
        rows.append(
            {
                "ExampleClass": label,
                "Gene": row["Gene"],
                "MutationPatientCount": int(row["MutationPatientCount"]),
                "MutationEvidenceStatus": row["MutationEvidenceStatus"],
                "LowFrequencyTargetScore": float(row["LowFrequencyTargetScore"]),
                "NonMutationSupport": float(row["nonmutation_support"]),
                "WeakNovelty": 0.0 if bool(row["train_driver"]) else 0.03,
                "MutationTargetReward": float(row["MutationTargetReward"]),
                "R_discovery_B": float(row["R_discovery_static"]),
            }
        )
    write_csv(OUT / "02_reward_design" / "reward_dry_run.csv", rows)
...
def write_reward_design_files(model):
    annotation_cols = [
        ...
        "LowFrequencyTargetScore",
        "MutationTargetReward",
        "nonmutation_support",
        "low_frequency_candidate",
        "supported_low_frequency",
        "R_relevance_static",
        "R_discovery_static",
        "R_robustness_static",
    ]
    annotation = model.static[annotation_cols].rename(columns={"mutation_frequency": "MutationFrequency"})
    write_csv(OUT / "02_reward_design" / "mutation_evidence_annotation.csv", annotation.to_dict("records"))

    score_rows = []
    for n in range(0, 101):
        score_rows.append(
            {
                "MutationPatientCount": n,
                "MutationFrequencyPct": mutation_frequency_pct(n, TOTAL_KIRC_TUMOR_SAMPLES),
                "MutationGroup": classify_mutation_frequency(n),
                "LowFrequencyTargetScore": low_frequency_target_score(n),
            }
        )
    write_csv(OUT / "02_reward_design" / "low_frequency_target_score.csv", score_rows)

    curve_x = [r["MutationPatientCount"] for r in score_rows]
    curve_y = [r["LowFrequencyTargetScore"] for r in score_rows]
    plt.figure(figsize=(8, 4))
    plt.plot(curve_x, curve_y, linewidth=2)
    for marker in [0, 1, 2, 18, 19]:
        plt.axvline(marker, color="#555555", linestyle="--", linewidth=0.8)
        plt.text(marker, low_frequency_target_score(marker) + 0.03, str(marker), ha="center", fontsize=8)
    plt.xlabel("MutationPatientCount")
    plt.ylabel("LowFrequencyTargetScore")
    plt.title("Missingness-aware low-frequency target score")
    plt.ylim(-0.05, 1.08)
    plt.tight_layout()
    plt.savefig(OUT / "02_reward_design" / "low_frequency_target_score_curve.png", dpi=180)
    plt.close()

    unit_tests = run_reward_unit_tests(model, score_rows)
    write_json(OUT / "02_reward_design" / "reward_unit_tests.json", unit_tests)
    write_reward_dry_run(model)
...
R_discovery_B(g) = LowFrequencyTargetScore(g) * NonMutationSupport(g) + WeakNovelty(g), clipped to [0,1].

LowFrequencyTargetScore is frozen before training:
...
N=0 currently means no observed functional mutation record in the current MC3 table, not confirmed measured/callable true zero. Therefore N=0 does not receive positive mutation-rarity targeting reward.

The fixed mutation-patient-count groups remain:

- very_low: MutationPatientCount < 2.
- low_frequency: 2 <= MutationPatientCount <= 18.
- high_frequency: MutationPatientCount >= 19.

NonMutationSupport(g) = mean(ExpressionPct, MethylationPct, NetworkSupportPct).

WeakNovelty(g) is the Stage4-A weak +0.03 term for genes not in Protocol B Train drivers.

Stage4-A used monotonic mutation rarity plus a low-frequency gate. Stage4-B V2 replaces that mutation targeting behavior with LowFrequencyTargetScore and keeps NonMutationSupport and WeakNovelty unchanged. R_relevance, R_robustness, preferences, backbone, DDQN, PER, soft update, action mask, seeds, and checkpoint rule are unchanged.
""",
    )
    write_text(
...
""",
    ...
    write_text(
        OUT / "02_reward_design" / "reward_design_v2_missingness_aware.md",
        f"""# Stage4-B: Missingness-Aware Low-Frequency-Targeted MORL

PREVIOUS_MUTATION_ABSENCE_AUDIT = FAIL

TRUE_ZERO_SEMANTICS_ASSUMED_FOR_N0 = NO

The previous audit is retained: the current files prove no observed functional mutation record for the 4017 absent model-universe genes, but do not prove measured/callable true zero. Stage4-B V1 stopped because it still gave N=0 a positive mutation rarity target. Stage4-B V2 no longer treats N=0 as positive mutation-rarity evidence.

Genes with N=0 stay in the 9039-gene universe and retain expression, methylation, network, patient coverage, and all original model features. They simply receive LowFrequencyTargetScore = 0.0.
...
S_LF(N) = 1.0 for 2 <= N <= 18.

S_LF(N) = exp(-(N - 18) / {LOW_FREQUENCY_TARGET_TAU}) for N >= 19.

## Discovery Reward

R_discovery_B = clip(S_LF * NonMutationSupport + WeakNovelty, 0, 1).

NonMutationSupport and WeakNovelty are copied from Stage4-A semantics. The old Stage4-A rarity and non-low-frequency 0.25 scaling are removed because the new score already encodes the target mutation interval and makes N=0 non-positive.

## Unchanged Conditions

R_relevance unchanged. R_robustness unchanged. Preference weights unchanged. Stage2_SimpleUnion backbone unchanged. DDQN/PER/Soft Update unchanged. Train/Validation split unchanged. Validation labels are used only for checkpoint selection and metrics, not reward. Test, historical Test, and external holdout are not read.
""",
    )
...
def write_scale_audit(model):
    ...
    write_csv(OUT / "02_reward_design" / "reward_component_distribution.csv", df.to_dict("records"))
    rows = []
    ...
    write_csv(OUT / "02_reward_design" / "reward_scale_summary.csv", rows)
    ...
    write_csv(OUT / "02_reward_design" / "reward_correlation.csv", corr_rows)
...
def write_formal_protocol(scale_rows, corr_rows):
    ...
    write_csv(OUT / "02_reward_design" / "seed_registry.csv", seed_rows)
    config = f"""# formal_config.yaml
PRIMARY_ENDPOINT: "Validation NDCG@150"
STAGE4B_BACKBONE: "Stage2_SimpleUnion"
STAGE4B_REWARD: "MISSINGNESS_AWARE_LOWFREQUENCY_TARGET"
feature_mode: "hybrid6_raw"
...
HIGH_FREQUENCY_DECAY_TAU: 10
"""
    write_text(OUT / "02_reward_design" / "formal_config_stage4B.yaml", config)


def smoke_and_gate(model):
    summaries = []
    for policy in POLICIES:
        run_dir = OUT / "03_smoke_test" / policy / "seed_42_smoke"
        if run_dir.exists() and not (run_dir / "summary.json").exists():
            i = 1
            while (run_dir.parent / f"seed_42_smoke_retry_{i:03d}").exists():
                i += 1
            run_dir = run_dir.parent / f"seed_42_smoke_retry_{i:03d}"
    ...
    write_csv(OUT / "03_smoke_test" / "smoke_summary.csv", rows)
    ...
    write_csv(OUT / "03_smoke_test" / "legacy_reward_regression.csv", regression)
    sanity = [
    ...
    write_csv(OUT / "03_smoke_test" / "reward_sanity_check.csv", sanity)
    report = ["# Stage4-B Smoke Test Report", "", f"SMOKE_STATUS = {'PASS' if passed else 'FAIL'}", ""]
    for row in rows:
        report.append(
            f"- {row['policy']} seed {row['seed']}: status={row['status']}, dead_end={row['dead_end_count']}, invalid={row['invalid_action_count']}, duplicate={row['duplicate_action_count']}, steps={row['steps']}"
        )
    write_text(OUT / "03_smoke_test" / "smoke_test_report.md", "\n".join(report) + "\n")
    return passed
...
def run_formal(model):
    ...
    for policy in POLICIES:
        for seed in SEEDS:
            run_dir = OUT / "04_formal_runs" / policy / f"seed_{seed}"
            summaries.append(build_run(policy, seed, run_dir, PPI_GRN_MESSAGE, model, smoke=False))
    write_csv(OUT / "04_formal_runs" / "formal_run_summary.csv", summaries)
    return summaries
...
def fisher_right_tail(a, b, c, d):
    ...
    def log_hyper(x):
        return math.lgamma(col1 + 1) - math.lgamma(x + 1) - math.lgamma(col1 - x + 1) + math.lgamma(n - col1 + 1) - math.lgamma(row1 - x + 1) - math.lgamma(n - col1 - row1 + x + 1) - (math.lgamma(n + 1) - math.lgamma(row1 + 1) - math.lgamma(n - row1 + 1))

    logs = [log_hyper(x) for x in range(a, max_x + 1)]
    m = max(logs)
    return float(math.exp(m) * sum(math.exp(v - m) for v in logs))


def top150_profile(stage, policy, seed, genes, metrics, static, model):
    top150 = genes[:150]
    ...
    for gene in top150:
        ...
        selected.append(idx)
    very_low = int((st["MutationGroup"] == "very_low").sum())
    lowfreq = int((st["MutationGroup"] == "low_frequency").sum())
    highfreq = int((st["MutationGroup"] == "high_frequency").sum())
    observed_one = int((st["MutationPatientCount"] == 1).sum())
    no_func_raw = int((st["MutationEvidenceStatus"] == "no_functional_but_raw_record").sum())
    no_raw = int((st["MutationEvidenceStatus"] == "no_raw_mutation_record").sum())
    supported_low = int(((st["MutationGroup"] == "low_frequency") & (st["nonmutation_support"] >= model.support_threshold)).sum())
    return {
        "stage": stage,
        "policy": policy,
        "seed": seed,
        "NDCG@150": float(metrics["NDCG@150"]),
        "Recall@150": float(metrics["Recall@150"]),
        "VeryLow@150": very_low,
        "LowFrequency@150": lowfreq,
        "HighFrequency@150": highfreq,
        "ObservedOneMutation@150": observed_one,
        "NoFunctionalButRawRecord@150": no_func_raw,
        "NoRawMutationRecord@150": no_raw,
        "SupportedLowFrequency@150": supported_low,
        "Top150LowFrequencyRate": lowfreq / 150.0,
...
def summarize_metric_rows(rows):
    ...
    metrics = [
        ...
        "LowFrequency@150",
        "HighFrequency@150",
        "ObservedOneMutation@150",
        "NoFunctionalButRawRecord@150",
        "NoRawMutationRecord@150",
        "SupportedLowFrequency@150",
        "Top150MeanHubPenalty",
        "Top150MeanRedundancy",
    ]
    for (stage, policy), sub in df.groupby(["stage", "policy"]):
        for metric in metrics:
            x = sub[metric].astype(float)
            out.append({"stage": stage, "policy": policy, "metric": metric, "mean": float(x.mean()), "SD": float(x.std(ddof=1)), "median": float(x.median()), "min": float(x.min()), "max": float(x.max())})
    return out
...
def analyze(model):
    ...
    for policy in POLICIES:
        for seed in SEEDS:
            for stage, base in [("Stage4-A", STAGE4A / "07_formal_runs"), ("Stage4-B", OUT / "04_formal_runs")]:
                run_dir = base / policy / f"seed_{seed}"
                ranking = load_ranking(run_dir)
                genes = [r["Gene"] for r in ranking]
                rankings[(stage, policy, seed)] = genes
                metrics = json.loads((run_dir / "validation_metrics_best.json").read_text(encoding="utf-8"))
                profile = top150_profile(stage, policy, seed, genes, metrics, static, model)
                all_profiles.append(profile)
                if stage == "Stage4-B":
                    annotated = []
                    for r in ranking[:150]:
                        s = static.loc[r["Gene"]]
                        item = dict(r)
                        item.update(
                            {
                                "MutationPatientCount": int(s["MutationPatientCount"]),
                                "MutationGroup": s["MutationGroup"],
                                "MutationEvidenceStatus": s["MutationEvidenceStatus"],
                                "LowFrequencyTargetScore": float(s["LowFrequencyTargetScore"]),
                                "nonmutation_support": float(s["nonmutation_support"]),
                                "supported_low_frequency": bool(s["supported_low_frequency"]),
                            }
                        )
                        annotated.append(item)
                    write_csv(OUT / "05_rankings" / f"{policy}_seed_{seed}_top150_annotated.csv", annotated)
            run_dir = OUT / "04_formal_runs" / policy / f"seed_{seed}"
    ...
    write_csv(OUT / "06_metrics" / "stage4B_per_seed_metrics.csv", [r for r in all_profiles if r["stage"] == "Stage4-B"])
    write_csv(OUT / "06_metrics" / "stage4B_summary.csv", summarize_metric_rows([r for r in all_profiles if r["stage"] == "Stage4-B"]))
    write_csv(OUT / "06_metrics" / "mutation_group_top150_summary.csv", all_profiles)
    write_csv(OUT / "06_metrics" / "mutation_evidence_status_top150_summary.csv", all_profiles)
    write_csv(OUT / "06_metrics" / "supported_lowfrequency_summary.csv", all_profiles)
    write_csv(OUT / "06_metrics" / "reward_contribution_summary.csv", contribution_rows)
    ...
    for policy in POLICIES:
        for seed in SEEDS:
            a = profile_df[(profile_df.stage == "Stage4-A") & (profile_df.policy == policy) & (profile_df.seed == seed)].iloc[0]
            b = profile_df[(profile_df.stage == "Stage4-B") & (profile_df.policy == policy) & (profile_df.seed == seed)].iloc[0]
            row = {
                "Policy": policy,
                "Seed": seed,
                "A_NDCG150": float(a["NDCG@150"]),
                "B_NDCG150": float(b["NDCG@150"]),
                "Delta_NDCG150": float(b["NDCG@150"] - a["NDCG@150"]),
                "A_Recall150": float(a["Recall@150"]),
                "B_Recall150": float(b["Recall@150"]),
                "Delta_Recall150": float(b["Recall@150"] - a["Recall@150"]),
                "A_LowFrequency150": int(a["LowFrequency@150"]),
                "B_LowFrequency150": int(b["LowFrequency@150"]),
                "Delta_LowFrequency150": int(b["LowFrequency@150"] - a["LowFrequency@150"]),
                "A_SupportedLowFrequency150": int(a["SupportedLowFrequency@150"]),
                "B_SupportedLowFrequency150": int(b["SupportedLowFrequency@150"]),
                "Delta_SupportedLowFrequency150": int(b["SupportedLowFrequency@150"] - a["SupportedLowFrequency@150"]),
                "A_VeryLow150": int(a["VeryLow@150"]),
                "B_VeryLow150": int(b["VeryLow@150"]),
                "Delta_VeryLow150": int(b["VeryLow@150"] - a["VeryLow@150"]),
                "A_NoRawMutationRecord150": int(a["NoRawMutationRecord@150"]),
                "B_NoRawMutationRecord150": int(b["NoRawMutationRecord@150"]),
                "Delta_NoRawMutationRecord150": int(b["NoRawMutationRecord@150"] - a["NoRawMutationRecord@150"]),
                "A_HighFrequency150": int(a["HighFrequency@150"]),
                "B_HighFrequency150": int(b["HighFrequency@150"]),
                "A_HubPenalty": float(a["Top150MeanHubPenalty"]),
                "B_HubPenalty": float(b["Top150MeanHubPenalty"]),
                "A_RedundancyPenalty": float(a["Top150MeanRedundancy"]),
                "B_RedundancyPenalty": float(b["Top150MeanRedundancy"]),
            }
            all_policy_rows.append(row)
            if policy == "Discovery":
                paired_rows.append({k: v for k, v in row.items() if k != "Policy"})
    write_csv(OUT / "07_comparison" / "stage4A_vs_stage4B_discovery_paired.csv", paired_rows)
    write_csv(OUT / "07_comparison" / "stage4A_vs_stage4B_all_policies.csv", all_policy_rows)

    background_low = int((static["MutationGroup"] == "low_frequency").sum())
    background_total = int(len(static))
    enrichment_rows = []
    for _, row in profile_df.iterrows():
        top_low = int(row["LowFrequency@150"])
        top_not = 150 - top_low
        rest_low = background_low - top_low
        rest_not = background_total - 150 - rest_low
        top_rate = top_low / 150.0
        bg_rate = background_low / background_total
        enrichment_rows.append(
            {
                "stage": row["stage"],
                "policy": row["policy"],
                "seed": int(row["seed"]),
                "BackgroundLowFrequencyRate": bg_rate,
                "Top150LowFrequencyRate": top_rate,
                "EnrichmentRatio": top_rate / bg_rate if bg_rate else "",
                "FisherExactRightTailP": fisher_right_tail(top_low, top_not, rest_low, rest_not),
            }
        )
    write_csv(OUT / "06_metrics" / "lowfrequency_enrichment.csv", enrichment_rows)

    overlap_rows = []
    for stage in ["Stage4-A", "Stage4-B"]:
        for seed in SEEDS:
            for a, b in [("Recovery", "Discovery"), ("Recovery", "Robustness"), ("Discovery", "Robustness")]:
                overlap_rows.append(
                    {
                        "stage": stage,
                        "seed": seed,
                        "policy_a": a,
                        "policy_b": b,
                        "Top50_Jaccard": jaccard(rankings[(stage, a, seed)], rankings[(stage, b, seed)], 50),
                        "Top100_Jaccard": jaccard(rankings[(stage, a, seed)], rankings[(stage, b, seed)], 100),
                        "Top150_Jaccard": jaccard(rankings[(stage, a, seed)], rankings[(stage, b, seed)], 150),
                        "RBO_Top150": rbo_score(rankings[(stage, a, seed)], rankings[(stage, b, seed)], 150),
                    }
                )
    write_csv(OUT / "07_comparison" / "policy_jaccard_comparison.csv", overlap_rows)

    trade_summary = classify_tradeoff(all_profiles, overlap_rows, contribution_rows, all_policy_rows)
    write_csv(OUT / "07_comparison" / "policy_tradeoff_summary.csv", [trade_summary])
...
"""
    write_text(OUT / "08_reports" / "preference_collapse_audit.md", collapse_md)
    make_figures(all_profiles, contribution_rows, overlap_rows)
    return trade_summary


def classify_tradeoff(profile_rows, overlap_rows, contribution_rows, comparison_rows):
    per = pd.DataFrame(profile_rows)
    b = per[per["stage"] == "Stage4-B"]
    comp = pd.DataFrame(comparison_rows)
    ov = pd.DataFrame(overlap_rows)
    contrib = pd.DataFrame(contribution_rows)
    mean_j = float(ov[ov["stage"] == "Stage4-B"]["Top150_Jaccard"].mean())
    collapse = "YES" if mean_j >= 0.95 else "NO"
    means = b.groupby("policy")["NDCG@150"].mean()
    recalls = b.groupby("policy")["Recall@150"].mean()
    supported_mean = b.groupby("policy")["SupportedLowFrequency@150"].mean()
    low_mean = b.groupby("policy")["LowFrequency@150"].mean()
    hub_mean = b.groupby("policy")["Top150MeanHubPenalty"].mean()
    red_mean = b.groupby("policy")["Top150MeanRedundancy"].mean()
    recovery_eff = "YES" if means.get("Recovery", 0) >= min(means.get("Discovery", 0), means.get("Robustness", 0)) else "UNCERTAIN"
    discovery_eff = "YES" if supported_mean.get("Discovery", 0) >= supported_mean.get("Recovery", 0) or low_mean.get("Discovery", 0) >= low_mean.get("Recovery", 0) else "UNCERTAIN"
    ...
    discovery_comp = comp[comp["Policy"] == "Discovery"]
    supported_delta_mean = float(discovery_comp["Delta_SupportedLowFrequency150"].mean())
    low_delta_mean = float(discovery_comp["Delta_LowFrequency150"].mean())
    no_raw_delta_mean = float(discovery_comp["Delta_NoRawMutationRecord150"].mean())
    ndcg_delta_mean = float(discovery_comp["Delta_NDCG150"].mean())
    recall_delta_mean = float(discovery_comp["Delta_Recall150"].mean())
    supported_improved = "YES" if supported_delta_mean > 0 else "NO"
    low_improved = "YES" if low_delta_mean > 0 else "NO"
    utility_preserved = "YES" if ndcg_delta_mean > -0.05 and recall_delta_mean > -0.10 else "NO"
    if collapse == "YES" or dominance == "YES":
        ...
        ready = "NO"
        status = "FAIL"
    if supported_improved == "YES" and utility_preserved == "YES" and collapse == "NO":
        status = "PASS"
    else:
        status = "FAIL"
    return {
        ...
        "RECOVERY_RECALL150": float(recalls.get("Recovery", np.nan)),
        "DISCOVERY_RECALL150": float(recalls.get("Discovery", np.nan)),
        "DISCOVERY_SUPPORTED_LOWFREQ150": float(supported_mean.get("Discovery", np.nan)),
        "DISCOVERY_LOWFREQ150": float(low_mean.get("Discovery", np.nan)),
        "DISCOVERY_DELTA_SUPPORTED_LOWFREQ150": supported_delta_mean,
        "DISCOVERY_DELTA_LOWFREQ150": low_delta_mean,
        "DISCOVERY_DELTA_NO_RAW_MUTATION_RECORD150": no_raw_delta_mean,
        "DISCOVERY_DELTA_NDCG150": ndcg_delta_mean,
        "DISCOVERY_DELTA_RECALL150": recall_delta_mean,
        "ROBUSTNESS_TOP150_JACCARD": float(ov[ov["stage"] == "Stage4-B"]["Top150_Jaccard"].mean()),
        "ROBUSTNESS_RBO150": float(ov[ov["stage"] == "Stage4-B"]["RBO_Top150"].mean()),
        "REWARD_COMPONENT_DOMINANCE": dominance,
        "PREFERENCE_COLLAPSE": collapse,
        "LOW_FREQUENCY_TARGETING_IMPROVED": low_improved,
        "SUPPORTED_LOW_FREQUENCY_TARGETING_IMPROVED": supported_improved,
...
def make_figures(profile_rows, contribution_rows, overlap_rows):
    figdir = OUT / "08_reports" / "figures"
    per = pd.DataFrame(profile_rows)
    b = per[per["stage"] == "Stage4-B"]
    contrib = pd.DataFrame(contribution_rows)
    ov = pd.DataFrame(overlap_rows)
    ...
    bar(b, "NDCG@150", "figure1_ndcg150.png", "Validation NDCG@150")
    bar(b, "Recall@150", "figure2_recall150.png", "Validation Recall@150")
    bar(b, "SupportedLowFrequency@150", "figure3_supported_lowfreq150.png", "Supported low-frequency @150")
    bar(b, "Top150MeanMutationFrequency", "figure4_top150_mean_mutation_frequency.png", "Top150 mean mutation frequency")
    plt.figure(figsize=(7, 4))
    ov[ov["stage"] == "Stage4-B"].groupby("policy_a")["Top150_Jaccard"].mean().plot(kind="bar")
    ...
    plt.close()
    plt.figure(figsize=(7, 4))
    ov[ov["stage"] == "Stage4-B"].groupby("policy_a")["RBO_Top150"].mean().plot(kind="bar")
    plt.ylabel("RBO Top150")
    plt.title("Policy pairwise RBO")
    plt.tight_layout()
    plt.savefig(figdir / "figure6_rbo_top150.png", dpi=180)
    plt.close()
    bar(b, "Top150MeanHubPenalty", "figure7_top150_hub_penalty.png", "Top150 PPI/GRN hub penalty")
    bar(b, "Top150MeanRedundancy", "figure8_top150_redundancy.png", "Top150 redundancy")
    ...
    trade = pd.DataFrame({
        ...
        "NDCG150": [b[b.policy == p]["NDCG@150"].mean() for p in ["Recovery", "Discovery", "Robustness"]],
        "SupportedLowFreq150": [b[b.policy == p]["SupportedLowFrequency@150"].mean() for p in ["Recovery", "Discovery", "Robustness"]],
        "LowHub": [1 - b[b.policy == p]["Top150MeanHubPenalty"].mean() for p in ["Recovery", "Discovery", "Robustness"]],
    })
...
def write_readme():
    write_text(OUT / "00_README.md", "# RL-GenRisk Stage4-B\n\nMissingness-Aware Low-Frequency-Targeted MORL on the frozen Stage2_SimpleUnion backbone.\n")


def write_final_report(trade_summary, formal_completed, scale_rows, corr_rows):
    paired_path = OUT / "07_comparison" / "stage4A_vs_stage4B_discovery_paired.csv"
    all_path = OUT / "07_comparison" / "stage4A_vs_stage4B_all_policies.csv"
    paired = pd.read_csv(paired_path) if paired_path.exists() else pd.DataFrame()
    all_policies = pd.read_csv(all_path) if all_path.exists() else pd.DataFrame()
    stage4b = pd.read_csv(OUT / "06_metrics" / "stage4B_per_seed_metrics.csv") if (OUT / "06_metrics" / "stage4B_per_seed_metrics.csv").exists() else pd.DataFrame()

    def mean_col(df, col):
        return "" if df.empty or col not in df else float(df[col].mean())

    def wlt(df, col):
        if df.empty or col not in df:
            return "0/0/0"
        x = df[col].astype(float)
        return f"{int((x > 0).sum())}/{int((x < 0).sum())}/{int((x == 0).sum())}"

    a_low = mean_col(paired, "A_LowFrequency150")
    b_low = mean_col(paired, "B_LowFrequency150")
    a_sup = mean_col(paired, "A_SupportedLowFrequency150")
    b_sup = mean_col(paired, "B_SupportedLowFrequency150")
    a_no_raw = mean_col(paired, "A_NoRawMutationRecord150")
    b_no_raw = mean_col(paired, "B_NoRawMutationRecord150")
    per_table = dataframe_to_markdown(stage4b) if len(stage4b) else "Formal runs not completed."
    all_table = dataframe_to_markdown(all_policies) if len(all_policies) else "Comparison not available."
    report = f"""# STAGE4B_COMPLETION_REPORT

## Why Audit FAIL No Longer Blocks V2

PREVIOUS_MUTATION_ABSENCE_AUDIT = FAIL is retained. The current files still cannot prove that N=0 genes are measured/callable true zero. Stage4-B V1 was stopped because it gave N=0 a positive mutation-rarity reward. Stage4-B V2 no longer gives N=0 positive mutation target reward, so the uncertain true-zero semantics no longer create a positive rarity incentive.

N=0 genes remain in the 9039-gene universe with expression, methylation, network, patient coverage, and model features intact. They may still be selected through other reward components, but not because N=0 is treated as rare mutation evidence.

## Method

Stage4-A = Monotonic Mutation Rarity MORL.

Stage4-B = Missingness-Aware Low-Frequency-Targeted MORL.

R_relevance unchanged. R_robustness unchanged. Preferences unchanged. Backbone = Stage2_SimpleUnion. DDQN/PER/Soft Update/action mask/checkpoint rule unchanged.

R_discovery_B = clip(LowFrequencyTargetScore * NonMutationSupport + WeakNovelty, 0, 1).

LowFrequencyTargetScore: N=0 -> 0.0; N=1 -> 0.5; 2 <= N <= 18 -> 1.0; N >= 19 -> exp(-(N-18)/10).

## Formal Results

FORMAL_RUNS_COMPLETED = {formal_completed} / 15

Stage4-B per-seed metrics:

{per_table}

Stage4-A vs Stage4-B all policies:

{all_table}

## Direct Answers

1. Retained original mutation absence audit conclusion: YES.
2. 2897 no-raw-record genes still have unknown true-zero status: YES.
3. N=0 S_LF is 0: YES.
4. N=1 S_LF is 0.5: YES.
5. 2-18 S_LF is 1.0: YES.
6. >=19 uses tau=10 smooth decay: YES.
7. tau was frozen before training: YES.
8. Only Discovery mutation targeting term changed: YES.
9. R_relevance unchanged: YES.
10. R_robustness unchanged: YES.
11. preference weights unchanged: YES.
12. backbone unchanged: YES.
13. DDQN/PER/Soft Update unchanged: YES.
14. Train/Validation split unchanged: YES.
15. Test read: NO.
16. Validation in reward: NO.
17. Formal runs completed: {formal_completed}/15.
18. Stage4-A Discovery LowFrequency@150 mean: {a_low}.
19. Stage4-B Discovery LowFrequency@150 mean: {b_low}.
20. Delta LowFrequency@150 mean: {mean_col(paired, 'Delta_LowFrequency150')}.
21. Stage4-A SupportedLowFrequency@150 mean: {a_sup}.
22. Stage4-B SupportedLowFrequency@150 mean: {b_sup}.
23. SupportedLowFrequency paired wins/losses/ties: {wlt(paired, 'Delta_SupportedLowFrequency150')}.
24. NoRawMutationRecord@150 change mean: {mean_col(paired, 'Delta_NoRawMutationRecord150')}.
25. NDCG@150 delta mean: {mean_col(paired, 'Delta_NDCG150')}.
26. Recall@150 delta mean: {mean_col(paired, 'Delta_Recall150')}.
27. HubPenalty delta mean: {'' if paired.empty else float((paired['B_HubPenalty'] - paired['A_HubPenalty']).mean())}.
28. RedundancyPenalty delta mean: {'' if paired.empty else float((paired['B_RedundancyPenalty'] - paired['A_RedundancyPenalty']).mean())}.
29. Preference collapse: {trade_summary.get('PREFERENCE_COLLAPSE', '')}.
30. Low-frequency enrichment output: 06_metrics/lowfrequency_enrichment.csv.
31. Supports missingness-aware low-frequency targeted reward: {trade_summary.get('STAGE4_STATUS', 'FAIL')}.
...
N1_MUTATION_TARGET_REWARD = 0.5
LOW_FREQUENCY_TARGET_RANGE = 2 <= N <= 18
LOW_FREQUENCY_TARGET_SCORE = 1.0
HIGH_FREQUENCY_DECAY_TAU = 10
STAGE4A_REWARD = MONOTONIC_RARITY
STAGE4B_REWARD = MISSINGNESS_AWARE_LOWFREQUENCY_TARGET
TEST_LABELS_READ = NO
HISTORICAL_TEST_READ = NO
EXTERNAL_HOLDOUT_READ = NO
VALIDATION_IN_REWARD = NO
HISTORICAL_STAGE4_OVERWRITTEN = NO
BACKBONE_CHANGED = NO
DDQN_CHANGED = NO
PER_CHANGED = NO
SOFT_UPDATE_CHANGED = NO
TRAIN_VALIDATION_SPLIT_CHANGED = NO
LOW_FREQUENCY_THRESHOLD_CHANGED_AFTER_RESULTS = NO
REWARD_PARAMETERS_CHANGED_AFTER_RESULTS = NO
FORMAL_RUNS = {formal_completed} / 15
PREFERENCE_COLLAPSE = {trade_summary.get('PREFERENCE_COLLAPSE', '')}
LOW_FREQUENCY_TARGETING_IMPROVED = {trade_summary.get('LOW_FREQUENCY_TARGETING_IMPROVED', '')}
SUPPORTED_LOW_FREQUENCY_TARGETING_IMPROVED = {trade_summary.get('SUPPORTED_LOW_FREQUENCY_TARGETING_IMPROVED', '')}
UTILITY_PRESERVED = {trade_summary.get('UTILITY_PRESERVED', '')}
STAGE4B_STATUS = {trade_summary.get('STAGE4_STATUS', 'FAIL')}

New and modified files are listed with SHA256 in 10_hashes/SHA256SUMS.txt.
"""
    write_text(OUT / "08_reports" / "STAGE4B_COMPLETION_REPORT.md", report)
...
def write_access_log():
    write_csv(OUT / "09_logs" / "data_access_manifest.txt", ACCESS_ROWS, ["timestamp", "file_or_resource", "purpose", "read_or_write", "category", "allowed_by_protocol", "notes"])
    write_text(
        OUT / "09_logs" / "execution_log.txt",
        f"Stage4-B Missingness-Aware Low-Frequency-Targeted MORL execution completed at {ts()}.\n"
        "Test labels read: NO. Historical Test read: NO. External holdout read: NO. Validation in reward: NO.\n",
    )


def write_integrity():
    targets = [path for path in OUT.rglob("*") if path.is_file() and path.name != "SHA256SUMS.txt"]
    lines = [f"{sha256_file(path)}  {display(path)}" for path in sorted(targets)]
    write_text(OUT / "10_hashes" / "SHA256SUMS.txt", "\n".join(lines) + "\n")
...
def main():
    ...
    write_readme()
    for path, purpose in [
        (MUTATION_ABSENCE_AUDIT, "read frozen Stage4-B mutation absence audit"),
    ...
    reward_tests = json.loads((OUT / "02_reward_design" / "reward_unit_tests.json").read_text(encoding="utf-8"))
    if reward_tests.get("status") != "PASS":
        trade_summary = {
            "STAGE4_STATUS": "FAIL",
            "PREFERENCE_COLLAPSE": "NA",
            "LOW_FREQUENCY_TARGETING_IMPROVED": "NA",
            "SUPPORTED_LOW_FREQUENCY_TARGETING_IMPROVED": "NA",
            "UTILITY_PRESERVED": "NA",
        }
        write_final_report(trade_summary, 0, [], [])
        write_access_log()
        write_integrity()
        return
...
FINAL_REPORT:
{display(OUT / '08_reports' / 'STAGE4B_COMPLETION_REPORT.md')}
